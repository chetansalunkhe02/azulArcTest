...
// import { getEmployeeList } from '../context/actions'
import axios from "axios"

export const EmployeeList = () => {
  const { employees, listEmployee, removeEmployee } = useContext(GlobalContext);
  useEffect(() => {
    axios.get('http://localhost:4001/employee/list')
      .then((response) => {
        listEmployee(response.data.data)
      }).catch((error) => {
  ...
  const deleteEmployee = (employeeId) => {
    ...
    axios.delete(`http://localhost:4001/employee/${employeeId}`)
      .then((response) => {
        alert(response.data.message)
        removeEmployee(response.data.data)
        // listEmployee(response.data.data)
      }).catch((error) => {
        console.log("error", error)
      })
